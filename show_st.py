...
from model_training import AutoIntMLPModel
...
@st.cache_resource
def load_data():
    project_path = '/Users/jake/Downloads/autoint'
    data_dir_nm = 'data/ml-1m'
    model_dir_nm = 'model'
    ...
    # 올바른 입력 데이터 형식으로 변환
    dummy_input = np.array([[0 for _ in range(len(field_dims))]])
    model(dummy_input)
    
    model.load_weights(f'{model_path}/autoInt_plus_model2.weights.h5')
